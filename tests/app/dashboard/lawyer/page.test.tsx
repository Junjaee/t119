...
// LawyerDashboard 페이지 테스트

import { describe, it, expect, vi } from 'vitest';
import { render, screen, waitFor } from '@testing-library/react';
import { QueryClient, QueryClientProvider } from '@tanstack/react-query';
import LawyerDashboard from '@/app/dashboard/lawyer/page';
...
vi.mock('@/features/dashboard/services/dashboardService', () => ({
  fetchLawyerDashboardData: vi.fn(() =>
    Promise.resolve({
      assignedCases: [{ id: '1', title: '사건A', severity: 'high' }],
...
describe('LawyerDashboard Page', () => {
  ...
  it('인증되지 않은 사용자 리디렉션', async () => {
    ...
    render(<LawyerDashboard />, { wrapper: createWrapper() });
  ...
  it('4개 위젯 렌더링', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('assigned-cases-widget')).toBeInTheDocument();
      expect(screen.getByTestId('active-consultations-widget')).toBeInTheDocument();
      expect(screen.getByTestId('rating-widget')).toBeInTheDocument();
      expect(screen.getByTestId('performance-stats-widget')).toBeInTheDocument();
    });
  });
